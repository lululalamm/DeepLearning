...
from datasets.supcon_dataset_custom import FaceDataset
...
def binary_func_sep(model, feat, scale, label, UUID, ce_loss_record_0, ce_loss_record_1, ce_loss_record_2):
    ...
    if indx_0.sum().item() > 0:
        logit_0 = model.module.fc0(feat[indx_0], scale[indx_0]).squeeze()
    ...
    if indx_1.sum().item() > 0:
        logit_1 = model.module.fc1(feat[indx_1], scale[indx_1]).squeeze()
    ...
    if indx_2.sum().item() > 0:
        logit_2 = model.module.fc2(feat[indx_2], scale[indx_2]).squeeze()
...
def main(args):
    ...
    if args.use_sampler:
        print("Train Sampler USE!")
        train_set,num_instances, weights = get_datasets(args.data_dir, FaceDataset, train=True, protocol=args.protocol, img_size=args.img_size, map_size=32, transform=train_transform, debug_subset_size=args.debug_subset_size,ca=args.ca,use_sampler=args.use_sampler)
        sampler = torch.utils.data.WeightedRandomSampler(weights, num_instances, replacement=True)
        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=False, num_workers=4,sampler=sampler)
    else:
        train_set = get_datasets(args.data_dir, FaceDataset, train=True, protocol=args.protocol, img_size=args.img_size, map_size=32, transform=train_transform, debug_subset_size=args.debug_subset_size,ca=args.ca,use_sampler=args.use_sampler)
        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=4)

    test_set = get_datasets(args.data_dir, FaceDataset, train=False, protocol=args.protocol, img_size=args.img_size, map_size=32, transform=test_transform, debug_subset_size=args.debug_subset_size,ca=args.ca)
    ...
    with open(os.path.join(args.result_path, args.result_name,"ori_title.txt"),'w') as f:
        f.writelines(args.ori_title)
    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        for i, sample_batched in enumerate(train_loader):
            ...
            if epoch >= args.align_epoch and args.align == 'v4':
                angle = model.module.update_weight_v4(alpha=args.alpha)
            else:
                angle = -1.0
        ...
        if epoch % epoch_test == epoch_test-1:
            ...
            if auc_test-HTER>=eva["best_auc"]-eva["best_HTER"]:
                ...
                model_path = os.path.join(model_root_path, "{}_p{}_{}_best.pth".format(args.model_type, args.ca,args.protocol))
                torch.save({
                    'epoch': epoch+1,
            ...
            if (epoch+1)%10==0:
                ...
                eva["tpr95"] = tpr
                eva["best_epoch"] = epoch+1
                model_path = os.path.join(model_root_path, "{}_p{}_{}_epoch_{}.pth".format(args.model_type, args.ca,args.protocol,epoch+1))
                torch.save({
                    'epoch': epoch+1,
                    'state_dict':model.state_dict(),
                    'optimizer':optimizer.state_dict(),
            ...
            model_path = os.path.join(model_root_path, "{}_p{}_{}_recent.pth".format(args.model_type, args.ca,args.protocol))
            torch.save({
                'epoch': epoch+1,
                'state_dict':model.state_dict(),
                'optimizer':optimizer.state_dict(),
...
def parse_args():
    ...
    # build dirs
    parser.add_argument('--data_dir', type=str, default="datasets/FAS", help='YOUR_Data_Dir') # csv root dir
    ...
    # custom
    parser.add_argument('--ca',default='crop',help='crop or align')
    parser.add_argument('--use_sampler',default=False,action='store_true')
...
if __name__ == '__main__':
    ...
    ori_title_sub = f"pre({pretrain_alias[args.pretrain]})_pgirm({args.align}-{args.align_epoch})_normfc({args.normfc})_bsz({args.batch_size})_rot({args.train_rotation})" + \
                       f"_smin({args.train_scale_min})_tscl({args.test_scale})_lr({args.base_lr})_alpha({args.alpha})_scale({args.scale})"+\
                       f"_floss({args.feat_loss})_flossw({args.feat_loss_weight})_tmp({args.temperature})_seed({args.seed})"

    args.ori_title = f"{args.protocol}_" + ori_title_sub+"\n"

    args.result_name = "{}_{}".format(args.protocol,args.ca)
