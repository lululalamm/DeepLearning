...
def image_preprocess(img_path,resize=112,mean_list=[0.485, 0.456, 0.406],std_list=[0.229, 0.224, 0.225]):
...
def get_args():
    ...
    parser.add_argument("--save_base",type=str,default="./save_results_re/")
...
if not args.save_name:
    save_txt = os.path.join(save_base,model_path.split("/")[-2]+"_test.txt")
    result_txt = os.path.join(save_base,model_path.split("/")[-2]+"_result.txt")
else:
    save_txt = os.path.join(save_base,args.save_name+"_test.txt")
    result_txt = os.path.join(save_base,args.save_name+"_result.txt")
...
# load test
if test_format=='txt':
    values = open(test_path,'r').readlines()
else:
...
with open(save_txt,'w') as f:
    for val in tqdm(values):
        if test_format=='txt':
            img_path = os.path.join(image_base,val.strip())
            gt = val.split(".jpg")[0].split("_")[-1]
        else:
            img_path = os.path.join(image_base,val[-2])
            gt = val[-2].split(".jpg")[0].split("_")[-1]
        
        if gt=='unmask':
            unmask_ct+=1
        else:
            mask_ct+=1
        gts.append(gt_dict[gt])
        ...
        if gt=='unmask' and lb=='unmask':
            unmask_true+=1
        if gt=='mask' and lb=='mask':
            mask_true+=1

        new_line = "{},{},{},{},{}\n".format(img_path,gt,gt_dict[gt],lb,np.argmax(output))
